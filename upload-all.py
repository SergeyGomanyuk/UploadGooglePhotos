...
def parse_args(arg_input=None):
    ...
    parser.add_argument('albums', metavar='albums',type=str, nargs='*',
...
def upload_photos(session, album_dirname):

    album_dir = os.fsencode(album_dirname)
    album_name = os.fsdecode(os.path.basename(album_dir))
    if(album_name == ".DS_Store"):
        return

    logging.info("Processing album: \'{}\'".format(album_name))
    ...
    for filename in os.listdir(album_dir):
        if(os.fsdecode(filename) == ".DS_Store"):
            continue
        photo_file_name = "{}/{}".format(os.fsdecode(album_dir), os.fsdecode(filename))
...
def main():
    ...
    for album_dirname in args.albums:

        session = get_authorized_session(args.auth_file)

        upload_photos(session, album_dirname)
